...
class MemeController {
  ...
  updateLikeNumberElement(likeNumber) {
    if (likeNumber < 0) {
      $("#meme-likes-id").css("color", "red");
    } else if (likeNumber > 0) {
      $("#meme-likes-id").css("color", "green");
    } else {
      $("#meme-likes-id").css("color", "black");
    }
...
function setUpButton(memeCtrl) {
  ...
  $("#next-image-id").on("click", function () {
    memeCtrl.nextMeme();
  });
  $("#prev-image-id").on("click", function () {
    memeCtrl.previousMeme();
  });
  $("#like-button-id").on("click", function () {
    memeCtrl.like();
  });
  $("#dislike-button-id").on("click", function () {
    memeCtrl.dislike();
  });
}
