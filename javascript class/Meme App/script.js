...
class MemeController {
  ...
  updateLikeNumberElement(likeNumber) {
    ...
    if (likeNumber < 0) {
      ...
      $("#meme-title-id").css({ "font-size": "1.2em", "font-style": "strong" });
    } else {
      $("#meme-likes-id").css({
        color: "black",
        "font-style": "normal",
      ...
      $("#meme-title-id").css({ "font-size": "1em", "font-style": "normal" });
    }
  }
  ...
  nextMeme() {
    ...
    const nextMemeID = (this._currentMeme.ID + 1) % this._memes.length;
    const nextMeme = this._memes[nextMemeID];
    if (nextMeme) {
  ...
  previousMeme() {
    ...
    let prevMemeID;
    if (this._currentMeme.ID === 0) {
      prevMemeID = this._memes.length - 1;
    } else {
      prevMemeID = (this._currentMeme.ID - 1) % this._memes.length;
    }
    const prevMeme = this._memes[prevMemeID];
    if (prevMeme) {
  ...
  like() {
    ...
    let currLikes = this._currentMeme._likeNumber;
    this._currentMeme.likeNumber = currLikes + 1;
    this.updateLikeNumberElement(this._currentMeme.likeNumber);
  }
  ...
  constructor(memes, currentMemeID) {
    ...
    // Otherwise, this will refer to the button object, not the controller object!
